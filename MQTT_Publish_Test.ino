...
// MQTT Publish
//const char* SubscribeToTemp = "esp/1/temperature"; //For publish temperature
//const char* SubscribeToHum = "esp/1/humidity"; //For publish humidity
...
PubSubClient pubClient(testClient);
...
//  Serial.println(thisBoard);
  ...
  // dht.setup(17);
  // use this instead: 
//  dht.setup(D3, DHTesp::DHT11); // Connect DHT sensor to GPIO D4
...
//  temperature = dht.getTemperature();
  ...
  potValue = map(potValue, 0, 1023, 0, 99);
//  Serial.println(potValue);
  
  long now = millis();
  if (now - lastTimeRecorded > 1000 || lastPotMessage.indexOf(potMessage) < 0) {
...
void reconnect() {
  ...
  while (!pubClient.connected()) {
    ...
    if (pubClient.connect("ESP_Client")) {
      ...
      pubClient.publish(SubscribeToPot, "Restart");
      // ... and resubscribe
